...
const activitySchema = new Schema({
  ...
  myBoughtSize: { type: Number, required: false },
});

export const getUserPositionModel = (walletAddress: string) => {
  const collectionName = `user_positions_${walletAddress}`;
  return mongoose.models[collectionName] || mongoose.model(collectionName, positionSchema, collectionName);
};

export const getUserActivityModel = (walletAddress: string) => {
  const collectionName = `user_activities_${walletAddress}`;
  return mongoose.models[collectionName] || mongoose.model(collectionName, activitySchema, collectionName);
};
